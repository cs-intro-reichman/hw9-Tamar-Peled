...
public class LinkedList {
	...
	public void add(int index, MemoryBlock block) {
		...
		if (index == 0) {// Adding at the beginning
			...
			Node prevFromIndex = getNode(index - 1);
			newNode.next = prevFromIndex.next;
			prevFromIndex.next = newNode;
	...
	public int indexOf(MemoryBlock block) {
		...
		for (int i = 0; i < size; i++) {
			MemoryBlock currentBlock = getBlock(i);
	...
	public void remove(Node node) {
		...
		if (this.first == node) {// Removing the first node
		...
		while (current != null) {
			if (current == node) {
				...
				if (current.next == null) {// Update last if removing the last node
					this.last = prev;
				}
				size--;
				return;
